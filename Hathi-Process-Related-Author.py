...
first column, searches each page for the table row that lists related 
author information (e.g., editors, commentators, etc.), and writes that 
information to a new CSV.

The reason for this is that some records originally gathered by DLL students
did not include this information, so we had to go get it.
...
with codecs.open('Hathi-output.csv','a',encoding='utf-8') as f:
    ...
    for i in HATHI_URLS:
        ...
        for row in rows:
            ...
            # Look for the row that contains "Related Names", then grab the data in the adjacent column.
            if stripped_label == 'Related Names':
                td = row.find('td')
                # Clean up the data.
                data = td.text
                regex = re.compile(r'[\n\r\t]')
